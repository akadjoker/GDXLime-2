 ...
 */
class DemoBS3DMeshStatic2 extends Screen
{
...
levelMesh.setShader(Gdx.SHADERPOINTLIGHT);
...
//node.setTexture(getTexture("data/models/textures/nskinwh.jpg",false,false,true));
	...
	override public function update(delta:Float):Void 
	{
		 ...
		 var speed:Float= 80;
		  ...
		  angle+= 0.5 *dt;
		  lightPosition =  new Vector3( 8.0 * Math.cos(angle), 4 * -Math.sin(angle), 8.0 * Math.sin(angle));// , 5.0 * sine);
		
		//lightPosition.copyFrom(camera.local_pos);
		
		
		 var shader:ShaderPointLight = cast(levelMesh.pipline, ShaderPointLight);
		 shader.position.copyFrom(lightPosition);
	   ...
	   if (keyPress(Keys.D))
		 ...
		 if (keyPress(Keys.S))
		 {
			 camera.Advance( -speed*dt);
		 	 ...
		 	 font.caption = Gdx.Instance().status + " range:" + shader.range+" intensity:" + shader.intensity;
