...
for (i in 0 ... 20)
{
	...
	//	camera.position.addInPlace(velocity);
		...
		new Vector3(camera.local_pos.x - 8, camera.local_pos.y - 8, camera.local_pos.z - 8), 
		new Vector3(camera.local_pos.x + 8, camera.local_pos.y + 8, camera.local_pos.z + 8));
		
		Colider.BoxTrace(bounds, camera.local_pos, ellipse, velocity,new Vector3(0,-0.2,0), 0.002);
	...
	/*
	   ...
	   if (keyPress(Keys.D))
		 ...
		 */
		 	 ...
		 	 font.caption = Gdx.Instance().status + " pos:" + camera.local_pos;
