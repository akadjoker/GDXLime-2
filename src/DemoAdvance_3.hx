...
import com.gdx.gl.RenderTexture;
 ...
 */
class DemoAdvance_3 extends Screen
{
	...
	var efect:RenderTexture;
	
	var renderCube:SceneNode;
	...
	override public function show():Void 
	{
		...
		camera2 = scene.addTargetCamera(0,1,-20,0,0,100);
...
node.scale(0.5);
	  ...
	  efect = new RenderTexture(256,256);
		
		
	  {
			var m = MeshCreator.createCube();
		    renderCube =  scene.addSceneNode(m);
			renderCube.setPos( -4, 4, 0);
			renderCube.setTexture(efect);
 ...
 font.caption = Gdx.Instance().status+Gdx.Instance().ParallaxMapScale ;
	...
	override public function render():Void 
	{
		 ...
		 renderCube.visible = false;
		 efect.Begin();
		 scene.render();
		 efect.End();
		 
		  scene.setCamera(camera);
		 renderCube.visible = true;
		 scene.render();
