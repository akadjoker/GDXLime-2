...
vertexbuffer.render(Gdx.gl.TRIANGLES, numParticles * 6);
		...
		Gdx.Instance().numTris += numParticles * 2 ;
	        ...
	        if(vertexbuffer.pipeline.vertexAttribute!=-1)  Gdx.gl.disableVertexAttribArray (vertexbuffer.pipeline.vertexAttribute);
 	        if(vertexbuffer.pipeline.texCoord0Attribute!=-1) Gdx.gl.disableVertexAttribArray (vertexbuffer.pipeline.texCoord0Attribute);
		    if (vertexbuffer.pipeline.colorAttribute!=-1) Gdx.gl.disableVertexAttribArray (vertexbuffer.pipeline.colorAttribute);	
	  }
