...
*/
class BilboardBatch  extends Node
{
  ...
  public var numParticles (default, null) :Int = 0;
    ...
    public function new (maxDecales:Int,?parent:Node=null,?id:Int=0,?Name:String="BillBoard") 
    {
	...
	public function addGrass( pos:Vector3, size:Float,life:Float=10,frame:Int=0):Void
	{
		addBillboard(pos, size, life, frame, 1);
		addBillboard(pos, size, life, frame, 3);
	}
...
vertexbuffer.render(Gdx.gl.TRIANGLES, numParticles * 6);
		...
		Gdx.Instance().numTris += numParticles * 2 ;
	         ...
	         Gdx.gl.disableVertexAttribArray (vertexbuffer.pipeline.vertexAttribute);
 	        if(vertexbuffer.pipeline.texCoord0Attribute>=0) Gdx.gl.disableVertexAttribArray (vertexbuffer.pipeline.texCoord0Attribute);
		    if (vertexbuffer.pipeline.colorAttribute >= 0) Gdx.gl.disableVertexAttribArray (vertexbuffer.pipeline.colorAttribute);	
	  }
