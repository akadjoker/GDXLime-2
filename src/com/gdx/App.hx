...
class App extends Application
{
   ...
   private var start:Bool;
	...
	{super.onWindowResize(width,height);if (!start) return;gdx.onresize(width, height); }

	public override  function onWindowFocusIn ():Void
	{super.onWindowFocusIn();if (!start) return;gdx.onShow(); }
	public override  function onWindowFocusOut ():Void 
	{super.onWindowFocusOut();if (!start) return;gdx.onHide(); }
	public  override function onWindowClose ():Void 
	{super.onWindowClose(); if (!start) return; gdx.onClose(); OnEnd(); }
