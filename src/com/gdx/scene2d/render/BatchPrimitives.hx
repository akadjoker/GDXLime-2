...
class BatchPrimitives 
{
	...
	this.vertexBuffer =  Gdx.gl.createBuffer();
	this.colorBuffer =  Gdx.gl.createBuffer();
	this.fvertexBuffer =  Gdx.gl.createBuffer();
	this.fcolorBuffer =  Gdx.gl.createBuffer();
	...
	Gdx.gl.bufferData(Gdx.gl.ARRAY_BUFFER,this.vertices , Gdx.gl.DYNAMIC_DRAW);
	colors = new Float32Array(capacity * 4 * 4);
	Gdx.gl.bindBuffer(Gdx.gl.ARRAY_BUFFER, this.colorBuffer);
	Gdx.gl.bufferData(Gdx.gl.ARRAY_BUFFER, this.colors , Gdx.gl.DYNAMIC_DRAW);
    
	fvertices = new Float32Array(capacity * 3 *4);
	Gdx.gl.bindBuffer(Gdx.gl.ARRAY_BUFFER, this.fvertexBuffer);
	Gdx.gl.bufferData(Gdx.gl.ARRAY_BUFFER,this.fvertices , Gdx.gl.DYNAMIC_DRAW);
	fcolors = new Float32Array(capacity * 4 * 4);
	Gdx.gl.bindBuffer(Gdx.gl.ARRAY_BUFFER, this.fcolorBuffer);
	Gdx.gl.bufferData(Gdx.gl.ARRAY_BUFFER, this.fcolors , Gdx.gl.DYNAMIC_DRAW);
...
public function fcolor(r:Float, g:Float,b:Float, ?a:Float =0.0)
	...
	{
	 Gdx.gl.bindBuffer(Gdx.gl.ARRAY_BUFFER, this.fvertexBuffer);	
     Gdx.gl.bufferSubData(Gdx.gl.ARRAY_BUFFER, 0, this.fvertices);
     Gdx.gl.vertexAttribPointer(shader.vertexAttribute, 3, Gdx.gl.FLOAT, false, 0, 0);
	 Gdx.gl.bindBuffer(Gdx.gl.ARRAY_BUFFER, this.fcolorBuffer);
	 Gdx.gl.bufferSubData(Gdx.gl.ARRAY_BUFFER, 0, this.fcolors);
     Gdx.gl.vertexAttribPointer(shader.colorAttribute, 4, Gdx.gl.FLOAT, false, 0, 0);
 	 Gdx.gl.drawArrays( Gdx.gl.TRIANGLES, 0, Std.int(fidxPos / 3));
	}
	 
	 if (idxPos >= 1)
	 {
	 Gdx.gl.bindBuffer(Gdx.gl.ARRAY_BUFFER, this.vertexBuffer);	
     Gdx.gl.bufferSubData(Gdx.gl.ARRAY_BUFFER, 0, this.vertices);
     Gdx.gl.vertexAttribPointer(shader.vertexAttribute, 3, Gdx.gl.FLOAT, false, 0, 0);
	 Gdx.gl.bindBuffer(Gdx.gl.ARRAY_BUFFER, this.colorBuffer);
	 Gdx.gl.bufferSubData(Gdx.gl.ARRAY_BUFFER, 0, this.colors);
     Gdx.gl.vertexAttribPointer(shader.colorAttribute, 4, Gdx.gl.FLOAT, false, 0, 0);
	 Gdx.gl.drawArrays(Gdx.gl.LINES, 0, Std.int(idxPos / 3));
 ...
 public function dispose():Void 
{
		...
		Gdx.gl.deleteBuffer(colorBuffer);
	
		this.fvertices = null;
		this.fcolors = null;
    	Gdx.gl.deleteBuffer(fvertexBuffer);
		Gdx.gl.deleteBuffer(fcolorBuffer);
