package com.gdx.Gdx.gl.shaders;
...
normalAttribute = Gdx.gl.getAttribLocation (shaderProgram, "inVertexNormal");
 ...
 LightMatrixUniform = Gdx.gl.getUniformLocation (shaderProgram, "LightMVMatrix");
 ...
 FBOTexture= Gdx.gl.getUniformLocation (shaderProgram, "uFBOTexture");
...
trace(vertexAttribute+","+colorAttribute+","+texCoord0Attribute );
	...
	override public function Bind(view:Matrix,proj:Matrix,world:Matrix):Void
	{
		  ...
		  if (shadow != null)
		  {
		   Gdx.gl.uniformMatrix4fv(LightMatrixUniform, false, shadow.lightMatrix.asArray());
		   shadow.bindBuffer(5);
		   Gdx.gl.uniform1i(FBOTexture, 5);
		   ...
		   Gdx.gl.uniformMatrix4fv(NormalMatrixUniform, false, normal.asArray());
	...
	{
		var u:GLUniformLocation = Gdx.gl.getUniformLocation(shaderProgram, s);
		Gdx.gl.uniform3f(u,v.x,v.y,v.z);
	}
	private function Uniform1f(a:Float,s:String):Void
	{
		var u:GLUniformLocation = Gdx.gl.getUniformLocation(shaderProgram, s);
		Gdx.gl.uniform1f(u, a);
	}
	
	private function Uniform4f(x:Float,y:Float,z:Float,w:Float,s:String):Void
	{
		var u:GLUniformLocation = Gdx.gl.getUniformLocation(shaderProgram, s);
		Gdx.gl.uniform4f(u,x,y,z,w);
	}
