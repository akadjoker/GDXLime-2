...
class Shader extends Buffer
{
	...
	{
	 	Gdx.gl.uniformMatrix4fv(boneMatrixUniform[index], false,  m.asArray());
	}
	
	public function setProjMatrix(m:Matrix):Void
	{
	    
	    Gdx.gl.uniformMatrix4fv(projMatrixUniform, false, m.asArray() );
	...
	Gdx.gl.uniformMatrix4fv(viewMatrixUniform, false,m.asArray());
	...
	public function setWorldMatrix(m:Matrix):Void
	{
	     ...
	     Gdx.gl.uniformMatrix4fv(worldMatrixUniform, false, m.asArray());
	...
	{
         Gdx.gl.uniform1i(MaterialType, m);
	...
	public function setTexture0(tex:Texture):Void
	{
		...
		if(tex != Gdx.Instance().currentBaseTextures[0] )
       {
			...
			tex.Bind(0);
			Gdx.gl.uniform1i(texture0Uniform, 0);
		...
		if(mat.textures[i] != Gdx.Instance().currentBaseTextures[i])
        {
			...
			switch(i)
			{
				...
				case 1:
					...
					Gdx.gl.uniform1i(texture1Uniform, 1);
				
					}
				 case 2:
					{
					mat.textures[i].Bind(2);		
					Gdx.gl.uniform1i(texture2Uniform, 2);
					}
				case 3:
					{
					mat.textures[i].Bind(3);		
					Gdx.gl.uniform1i(texture3Uniform, 3);
					}	
			}
