...
*/
class ShaderFog extends Shader
{
...
colorAttribute = Gdx.gl.getAttribLocation (shaderProgram, "inVertexColor");
 ...
 FogInfos=Gdx.gl.getUniformLocation (shaderProgram, "uFogInfos");
 FogColor = Gdx.gl.getUniformLocation (shaderProgram, "uFogColor");
 setMaterialType(0);
...
trace(vertexAttribute+","+colorAttribute+","+texCoord0Attribute );
	...
	{
		    var color:Color3 =  Gdx.Instance().fogColor;
			Gdx.gl.uniform3f(FogColor, color.r, color.g, color.b);
		    Gdx.gl.uniform4f(FogInfos, Gdx.Instance().fogMode,Gdx.Instance().fogStart,Gdx.Instance().fogEnd,Gdx.Instance().fogDensity); 
