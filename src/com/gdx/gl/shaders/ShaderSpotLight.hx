...
*/
class ShaderSpotLight extends Shader
{
...
normalAttribute= Gdx.gl.getAttribLocation (shaderProgram, "inVertexNormal");
 ...
 normalMatrixUniform= Gdx.gl.getUniformLocation (shaderProgram, "NormalMatrix");
 ...
 specularIntensityUniform= Gdx.gl.getUniformLocation (shaderProgram, "specularIntensity");
 specularPowerIntensityUniform= Gdx.gl.getUniformLocation (shaderProgram, "specularPower");
 ambientLightUniform = Gdx.gl.getUniformLocation (shaderProgram, "ambientLight");



constantUniform= Gdx.gl.getUniformLocation (shaderProgram, "spotLight.pointLight.atten.constant");
linearUniform= Gdx.gl.getUniformLocation (shaderProgram, "spotLight.pointLight.atten.linear");
exponentUniform = Gdx.gl.getUniformLocation (shaderProgram, "spotLight.pointLight.atten.exponent");

//base
colorUniform = Gdx.gl.getUniformLocation (shaderProgram, "spotLight.pointLight.base.color");
intensityUniform = Gdx.gl.getUniformLocation (shaderProgram, "spotLight.pointLight.base.intensity");

positionLightUniform = Gdx.gl.getUniformLocation (shaderProgram, "spotLight.pointLight.position");
rangeUniform = Gdx.gl.getUniformLocation (shaderProgram, "spotLight.pointLight.range");
directionUniform = Gdx.gl.getUniformLocation (shaderProgram, "spotLight.direction");
cutoffUniform= Gdx.gl.getUniformLocation (shaderProgram, "spotLight.cutoff");
...
trace("ShaderSpotlight:",vertexAttribute+","+colorAttribute+","+texCoord0Attribute+","+normalAttribute );
    ...
    override	public function Bind(view:Matrix,proj:Matrix,world:Matrix):Void
	{
     super.Bind(view, proj, world);
  	 Gdx.gl.uniform3f(ambientLightUniform, ambient.r, ambient.g, ambient.b);
     
	//base
	 Gdx.gl.uniform3f(colorUniform,color.r,color.g,color.b);
     Gdx.gl.uniform1f(intensityUniform, intensity);
	 //specular
	 Gdx.gl.uniform1f(specularIntensityUniform, specularIntensity);
	 Gdx.gl.uniform1f(specularPowerIntensityUniform, specularPower);
    //atte
     var a:Float = exponent;
	 var b:Float = linear;
	 var c:Float = constant - COLOR_DEPTH  * intensity * color.Max();
     range =  (( -b + Math.sqrt(b * b - 4 * a * c)) / (2 * a));
     Gdx.gl.uniform1f(rangeUniform, range);

         Gdx.gl.uniform1f(constantUniform, constant);
	     Gdx.gl.uniform1f(linearUniform, linear);
		 Gdx.gl.uniform1f(exponentUniform, exponent);
	     Gdx.gl.uniform1f(cutoffUniform, cutoff);
		 Gdx.gl.uniform3f(eyePositionIntensityUniform, camarePosition.x, camarePosition.y, camarePosition.z);
		 direction.normalize();
		 Gdx.gl.uniform3f(directionUniform, direction.x, direction.y, direction.z);
	     Gdx.gl.uniform3f(positionLightUniform, position.x, position.y, position.z);	 

	}
	
    public function setLightData(light:SpotLight,camPos:Vector3):Void
	{
